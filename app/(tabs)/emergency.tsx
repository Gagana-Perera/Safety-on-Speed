...
export default function EmergencyServices() {
  // Router handles navigation to Map and back actions.
  const router = useRouter();
  // Theme provides dynamic colors for light/dark rendering.
  const { theme } = useTheme();

  // if you dont allow the location access in the home page ,then ask fro the location access in  here.
      ...
      [
        {
          ...
          onPress: () => {
            void (async () => {
              ...
              try {
                //remove earlier result  from the asynsc storage update it with new user input.
  ...
  // Stores the latest known user coordinates used for nearby search.
  const [userLocation, setUserLocation] = useState<{
    lat: number;
    lng: number;
  } | null>(null);
  // Human-readable location failure state shown under the page title.
  ...
  >({});
  // Prevents duplicate phone-detail calls for the same place.
  ...
  useEffect(() => {
    // Main startup flow: permission checks + initial and live GPS updates.
    (async () => {
      try {
        setGpsError(null);
  ...
  useEffect(() => {
    // When the app returns to foreground, retry GPS bootstrap if needed.
  ...
  const ensureNearestPlaceId = async (
    ...
    // Only place cards need Places API lookup.
  ...
  const ensurePlacePhone = async (
    ...
    // Key ensures uniqueness for each card/place combination.
  ...
  const refreshPlaceItem = (item: ServiceItem) => {
    // Guard clauses keep this function cheap if prerequisites are missing.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await Promise.allSettled(
        // Warm up both nearest placeId and phone in background.
  ...
  const handleMapAction = async (item: ServiceItem) => {
    ...
    const cachedPlaceId = placeCacheRef.current[item.id]?.placeId || null;
    // Fallback keyword lets Map open category search when placeId is delayed.
    ...
    if (!quick.done) {
      // Navigate immediately, then refine to exact place once lookup resolves.
      router.push({
        pathname: "/map",
  ...
  const makePhoneCall = async (phoneNumber: string) => {
    // Normalize and validate before opening dialer intent.
  ...
  const renderCard = (item: ServiceItem, opts?: { marginBottom?: number }) => {
    // Button-specific loading control to keep other cards interactive.
  ...
  return (
    ...
    >
      <ScrollView
        // Screen is intentionally scrollable for smaller devices/accessibility text sizes.
        ...
        <View style={globalStyles.extraSection}>
          <Text
            style={[
              ...
              {
                color:
                  theme.mode === "light" ? "#555" : "rgba(255,255,255,0.75)",
              },
            ]}
          >
            Emergency Hotlines
          ...
          <View style={globalStyles.extraGrid}>
            {SERVICES.filter((s) => s.category === "hotline").map(
              (item, index) =>
                renderCard(item, { marginBottom: index < 2 ? 16 : 12 }),
            )}
          </View>
        </View>
